...
{
	if(AntiAnim[playerid] > gettime() ) {
	    SendClientMessage(playerid, COLOR_GRAD2, "Ban can phai doi trong vai giay de tiep tuc hanh dong!");
	    return 1;
	}
	gPlayerUsingLoopingAnim[playerid] = 1;
	ApplyAnimation(playerid, animlib, animname, fDelta, loop, lockx, locky, freeze, time, forcesync);
	TextDrawShowForPlayer(playerid,txtAnimHelper);
	AntiAnim[playerid] = gettime() + 5;
	return 1;
}
...
hook OnGameModeInit()
{
	...
	"~r~~k~~PED_SPRINT~ ~w~de dung hanh dong");
...
{
	SendClientMessage(playerid, COLOR_GREEN, "Dang sach hanh dong:");
	...
	SendClientMessage(playerid, COLOR_GREEN, "An nut F de dung hanh dong.");
	return 1;
}
...
CMD:seat(playerid, params[])
{
    ...
    default: SendClientMessage(playerid, COLOR_WHITE, "USAGE: /seat [1-2]");
    }
    return 1;
}
...
CMD:sleep(playerid, params[])
{
	if(!IsAblePedAnimation(playerid)) return 1;
	new string[128];
	format(string, sizeof(string), "{FF8000}* {C2A2DA}%s vua su dung /sleep", GetPlayerNameEx(playerid));
...
CMD:shake(playerid, params[])
{
    ...
    default: SendClientMessage(playerid, COLOR_WHITE, "USAGE: /shake [1-3]");
    }
    return 1;
}
...
CMD:handsup(playerid, params[])
{
    ...
    cmd_me(playerid, "vua su dung handsup.");
	return 1;
}
...
/*CMD:piss(playerid, params[])
{
	if(!IsAblePedAnimation(playerid)) return 1;
   	...
   	cmd_me(playerid, "vua su dung /piss.");
	return 1;
}*/
...
CMD:fallover(playerid, params[])
{
	...
	cmd_me(playerid, "vua su dung fallover.");
	return 1;
}
...
{
if(!IsAblePedAnimation(playerid)) return 1;
PlayAnimEx(playerid, "MISC", "Hiker_Pose_L", 4.0, 0, 1, 1, 1, 0, 1);
return 1;
}
...
CMD:lowrider(playerid, params[])
{
	...
	{
		SendClientMessage(playerid, COLOR_GRAD2, "Dang su dung hanh dong tuong tac.");
	}
	return 1;
}
