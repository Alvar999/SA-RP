...
#define 			JOB_LUATSU  			(6301)

new JobLuatSuID = 2;

stock ShowJOB_LUATSU(playerid) {
	...
	ShowPlayerDialog(playerid, JOB_LUATSU, DIALOG_STYLE_LIST, "Tro giup cong viec", name, "Tim viec", "Huy Bo");
	return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
	if(PRESSED(KEY_YES)){
		if(ActorPos(playerid, ActorJob[1])) return ShowJOB_LUATSU(playerid);
	}
	return 1;
}

hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	if(dialogid == JOB_LUATSU && response == 1) {
		if(listitem == 0) {
		...
		if(listitem == 1) {
			if(PlayerInfo[playerid][pDonateRank] == 0) {
				if(PlayerInfo[playerid][pJob] == 0) {
					PlayerInfo[playerid][pJob] = JobLuatSuID;
					new name[1280];
				...
				if(PlayerInfo[playerid][pJob] == 0) {
					...
					PlayerInfo[playerid][pJob2] = JobLuatSuID;
					new name[1280];
