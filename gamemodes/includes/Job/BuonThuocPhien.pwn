...
#define 			JOB_BuonThuocPhien  			(6303)

new JobBuonThuocPhienID = 4;

stock ShowJOB_BuonThuocPhien(playerid) {
	...
	ShowPlayerDialog(playerid, JOB_BuonThuocPhien, DIALOG_STYLE_LIST, "Tro giup cong viec", name, "Tim viec", "Huy Bo");
	return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
	if(PRESSED(KEY_YES)){
		if(ActorPos(playerid, ActorJob[3])) return ShowJOB_BuonThuocPhien(playerid);
		if(ActorPos(playerid, ActorJob[23])) return  ShowPlayerDialog(playerid, DIALOG_LICENSE_BUY, DIALOG_STYLE_LIST, "Chon loai giay phep ban muon mua.", "Giay phep lai xe ($5,000)\r\nGiay phep lai thuyen ($5,000)\r\nGiay phep lai may bay ($25,000)\r\nGiay phep lai taxi ($35,000)", "Mua", "Huy bo");
	}
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	if(dialogid == JOB_BuonThuocPhien && response == 1) {
		if(listitem == 0) {
		...
		if(listitem == 1) {
			if(PlayerInfo[playerid][pDonateRank] == 0) {
				if(PlayerInfo[playerid][pJob] == 0) {
					PlayerInfo[playerid][pJob] = JobBuonThuocPhienID;
					new name[1280];
				...
				if(PlayerInfo[playerid][pJob] == 0) {
					...
					PlayerInfo[playerid][pJob2] = JobBuonThuocPhienID;
					new name[1280];
