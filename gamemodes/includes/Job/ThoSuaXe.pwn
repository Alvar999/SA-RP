...
#define 			JOB_ThoSuaXe  			(9315)

new JobThoSuaXeID = 7;

stock ShowJOB_ThoSuaXe(playerid) {
	...
	ShowPlayerDialog(playerid, JOB_ThoSuaXe, DIALOG_STYLE_LIST, "Tro giup cong viec", name, "Tim viec", "Huy Bo");
	return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
	if(PRESSED(KEY_YES)){
		if(ActorPos(playerid, ActorJob[20])) return ShowJOB_ThoSuaXe(playerid);
	}
	return 1;
}

hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	if(dialogid == JOB_ThoSuaXe && response == 1) {
		if(listitem == 0) {
		...
		if(listitem == 1) {
			if(PlayerInfo[playerid][pDonateRank] == 0) {
				if(PlayerInfo[playerid][pJob] == 0) {
					PlayerInfo[playerid][pJob] = JobThoSuaXeID;
					new name[1280];
				...
				if(PlayerInfo[playerid][pJob] == 0) {
					...
					PlayerInfo[playerid][pJob2] = JobThoSuaXeID;
					new name[1280];
