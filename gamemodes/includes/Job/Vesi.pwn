...
#define 			JOB_VeSi  				(6312)

new JobVeSiID = 8;

stock ShowJOB_Vesi(playerid) {
	...
	ShowPlayerDialog(playerid, JOB_VeSi, DIALOG_STYLE_LIST, "Tro giup cong viec", name, "Tim viec", "Huy Bo");
	return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
	if(PRESSED(KEY_YES)){
		if(ActorPos(playerid, ActorJob[19])) return ShowJOB_Vesi(playerid);
	}
	return 1;
}

hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	if(dialogid == JOB_VeSi && response == 1) {
		if(listitem == 0) {
		...
		if(listitem == 1) {
			if(PlayerInfo[playerid][pDonateRank] == 0) {
				if(PlayerInfo[playerid][pJob] == 0) {
					PlayerInfo[playerid][pJob] = JobVeSiID;
					new name[1280];
				...
				if(PlayerInfo[playerid][pJob] == 0) {
					...
					PlayerInfo[playerid][pJob2] = JobVeSiID;
					new name[1280];
