...
#define 			JOB_GaiDiem  			(6302)

new JobGaiDiemID = 3;

stock ShowJOB_GaiDiem(playerid) {
	...
	ShowPlayerDialog(playerid, JOB_GaiDiem, DIALOG_STYLE_LIST, "Tro giup cong viec", name, "Tim viec", "Huy Bo");
	return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
	if(PRESSED(KEY_YES)){
		if(ActorPos(playerid, ActorJob[2])) return ShowJOB_GaiDiem(playerid);
	}
	return 1;
}

hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
	if(dialogid == JOB_GaiDiem && response == 1) {
		if(listitem == 0) {
		...
		if(listitem == 1) {
			if(PlayerInfo[playerid][pDonateRank] == 0) {
				if(PlayerInfo[playerid][pJob] == 0) {
					PlayerInfo[playerid][pJob] = JobGaiDiemID;
					new name[1280];
				...
				if(PlayerInfo[playerid][pJob] == 0) {
					...
					PlayerInfo[playerid][pJob2] = JobGaiDiemID;
					new name[1280];
